    ...
    namespace EcoRoute.UI.Services.WasteBinServices
    {
        public class WasteBinService
        {
            ...
            public async Task<ResultWasteBinDto> GetWasteBinByIdAsync(Guid id)
            {
                var token = await _localStorage.GetItemAsync<string>("authToken");
                _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);

                var response = await _httpClient.GetFromJsonAsync<ResultWasteBinDto>($"services/datacollection/WasteBins/{id}");
                return response;
            }
            ...
            public async Task<bool> DeleteWasteBinAsync(Guid id)
            {
                ...
                var response = await _httpClient.DeleteAsync($"services/datacollection/WasteBins?id={id}");
                return response.IsSuccessStatusCode;
            }
        }
    }
